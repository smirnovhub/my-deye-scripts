...
// Code from http://code.google.com/p/minify/
function isBuggyIe($ua)
{
...
if(
	(isset($headers['User-Agent']) == true) &&
	(isBuggyIe($headers['User-Agent']) == false) &&
	(isset($headers['Accept-Encoding']) == true) &&
	(strpos($headers['Accept-Encoding'], 'gzip') !== false)
	)
{
	define('GZIP_OK', true);
}
...
class JsHttpRequest
{
    ...
    function _obHandler($text)
    {
		...
		if(defined('GZIP_OK') == true)
		{
			...
			$contents .= pack('V', $crc);
			$contents .= pack('V', $size);

			$text = "\x1f\x8b\x08\x00\x00\x00\x00\x00".$contents;
