...
class DeyeProxyConfig:
  def __init__(self):
    self.logger_host_var = DeyeProxyEnvVar("LOGGER_HOST", "", "IP/Hostname of the inverter logger (mandatory)")
    ...
    self.LOGGER_HOST = self.logger_host_var.value
  ...
  def validate_or_exit(self):
    if not self.LOGGER_HOST:
      print(f"Environment variable '{self.logger_host_var.name}' is not set. Exiting.")
