...
class DeyeTestHelper:
  ...
  def get_random_by_register_type(
    ...
    if isinstance(register.value, DeyeBaseEnum):
      ...
      return DeyeTestHelper._handle_temperature_register(register)

    elif isinstance(register, SignedFloatDeyeRegister):
      return DeyeTestHelper._handle_signed_float_register(register)
  ...
  def _handle_temperature_register(register: TemperatureDeyeRegister) -> DeyeRegisterRandomValue:
    val = int(random.uniform(0, (DeyeTestHelper.unsigned_max_value + register.shift) / register.scale) * register.scale)
    value = DeyeUtils.custom_round((val + register.shift) / register.scale)
  ...
  def _handle_signed_float_register(register: SignedFloatDeyeRegister) -> DeyeRegisterRandomValue:
    val = int(
      random.uniform(DeyeTestHelper.signed_min_value / register.scale, DeyeTestHelper.signed_max_value / register.scale)
      * register.scale)
    value = DeyeUtils.custom_round(val / register.scale)
    return DeyeRegisterRandomValue(register, value, [DeyeUtils.to_unsigned(val)])

  @staticmethod
  def _handle_float_register(register: FloatDeyeRegister) -> DeyeRegisterRandomValue:
    val = int(random.uniform(0, DeyeTestHelper.unsigned_max_value / register.scale) * register.scale)
    value = DeyeUtils.custom_round(val / register.scale)
    return DeyeRegisterRandomValue(register, value, [val])

  @staticmethod
  def _handle_signed_int_register(register: SignedIntDeyeRegister) -> DeyeRegisterRandomValue:
    value = random.randint(DeyeTestHelper.signed_min_value, DeyeTestHelper.signed_max_value)
    return DeyeRegisterRandomValue(register, value, [DeyeUtils.to_unsigned(value)])

  @staticmethod
  def _handle_int_register(register: IntDeyeRegister) -> DeyeRegisterRandomValue:
    value = random.randint(0, DeyeTestHelper.unsigned_max_value)
  ...
  def _handle_time_of_use_int_register(register: TimeOfUseIntWritableDeyeRegister) -> DeyeRegisterRandomValue:
    ...
    for i in range(register.quantity):
      val = random.randint(0, DeyeTestHelper.unsigned_max_value)
      values.append(val)
  ...
  def _handle_system_time_writable_register(register: SystemTimeWritableDeyeRegister) -> DeyeRegisterRandomValue:
    ...
    # Year is offset by 2000 to match your `DeyeUtils.to_inv_time` format
    as_ints = [
      random_dt.year - 2000,
      random_dt.month,
      random_dt.day,
  ...
  def get_random_by_register_value_type(register: DeyeRegister, skip_zero: bool = False) -> Optional[str]:
    if isinstance(register.value, int):
      return DeyeTestHelper._handle_int_value(register, skip_zero)

    elif isinstance(register.value, float):
      return DeyeTestHelper._handle_float_value(register, skip_zero)

    elif isinstance(register.value, DeyeBaseEnum):
      return DeyeTestHelper._handle_enum_value(register.value, skip_zero)

    elif isinstance(register.value, datetime):
      return DeyeTestHelper._handle_datetime_value()
